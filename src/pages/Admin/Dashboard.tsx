...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-[#efede6] overflow-hidden font-sans text-stone-900">
      
      {/* LEFT SIDEBAR */}
      <aside className="w-72 bg-stone-900 text-stone-100 flex flex-col shrink-0 border-r border-stone-800">
         {/* Logo Area */}
         ...
         <div className="flex-1 overflow-y-auto py-4 px-3 space-y-1 custom-scrollbar">
            {CATEGORIES.map(cat => (
               <button
                 key={cat.id}
                 onClick={() => { setActiveCategory(cat.id as CategoryId); setSelectedItemId(null); }}
                 className={cn(
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar relative">
         {/* Top Bar (Contextual) */}
         <div className="sticky top-0 z-10 bg-[#efede6]/90 backdrop-blur-md border-b border-stone-200 px-8 py-4 flex justify-between items-center">
             <div className="flex items-center gap-2 text-sm font-mono text-stone-500">
                 <span>Admin</span>
                 <ChevronRight size={14} />
                 <span className="uppercase text-stone-800 font-bold">{CATEGORIES.find(c => c.id === activeCategory)?.label}</span>
             </div>
