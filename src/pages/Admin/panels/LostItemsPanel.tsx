       ...
       <div className="grid grid-cols-1 gap-4">
           ...
           {items.map(item => (
               <div key={item.id} className={cn("bg-white p-4 rounded-lg border shadow-sm flex flex-col gap-3 group relative overflow-hidden", item.status === 'resolved' ? 'opacity-70 bg-stone-50' : '')}>
                   ...
                   {/* Private Notes Section for Admin */}
                   {item.notes && item.notes.length > 0 && (
                       <div className="mt-2">
                           ...
                           >
                               <MessageCircle size={14}/> {item.notes.length} Gizli Not Mevcut (Görüntüle)
                           </button>
                           {expandedNotesId === item.id && (
                               <div className="mt-2 space-y-2 pl-4 border-l-2 border-blue-100">
                                   {item.notes.map((note: any, idx: number) => (
                                       <div key={idx} className="bg-blue-50 p-2 rounded text-xs text-stone-700">
                                           <p className="font-bold mb-1">{note.authorName} ({note.authorContact}):</p>
                                           <p>{note.content}</p>
                                           <div className="text-[10px] text-stone-400 mt-1">{formatDate(note.createdAt)}</div>
                                       </div>
                                   ))}
                               </div>
                           )}
