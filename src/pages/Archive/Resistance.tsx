...
const Resistance: React.FC = () => {
    ...
    return (
        <div className="w-full flex flex-col bg-[#f5f5f4] min-h-[calc(100vh-64px)] text-[#292524] relative">

            {/* HEADER */}
            <div className="px-4 pt-20 pb-4 md:p-4 flex justify-between items-start">
                <button
                    onClick={handleBack}
                    className="flex items-center gap-2 text-stone-600 hover:text-stone-900 transition-colors bg-white hover:bg-stone-50 px-4 py-2 border border-stone-200 shadow-sm rounded-full"
                >
                    <ArrowLeft size={16} />
            ...
            {createPortal(
                <AnimatePresence>
                    {viewerContent && (
                        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/95 md:backdrop-blur-md p-4">
