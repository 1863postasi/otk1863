...
const { useSearchParams, useNavigate } = router;

import StoryCard from '../../components/Archive/Cards/StoryCard';
...
const Roots: React.FC = () => {
    ...
    const openStory = React.useCallback((story: Story) => {
    ...
    return (
        <div className="min-h-full bg-[#f5f5f4] text-stone-900 font-sans relative pb-20">
            ...
            <div className="sticky top-0 z-40 bg-[#f5f5f4]/95 backdrop-blur-md border-b border-stone-200 shadow-sm transition-all">
                ...
                <div className="px-4 pt-16 pb-3 md:py-3 max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-y-3 gap-x-4">

                    {/* Title & Mobile Back */}
                    <div className="flex items-center gap-3 shrink-0 w-full md:w-auto">
                        <button onClick={() => navigate('/arsiv')} className="p-2 -ml-2 hover:bg-stone-200 rounded-full transition-colors md:hidden">
                            <ArrowLeft size={20} />
                        </button>
                        {/* Desktop Back */}
                        <button
                        ...
                        >
                            {displayedStories.map((story) => (
                                <StoryCard
                                    key={story.id}
                                    story={story}
                                    isSaved={userProfile?.savedRootIds?.includes(story.id) || false}
                                    onClick={openStory}
                                />
                            ))}
                        </motion.div>
