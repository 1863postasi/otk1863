...
import { Lock, ArrowRight, Play, Star } from 'lucide-react';
...
const GameCard: React.FC<GameCardProps> = ({ 
  ...
  return (
    <motion.div
      whileHover={!isLocked ? { y: -4, boxShadow: "0 10px 30px -10px rgba(0,0,0,0.1)" } : {}}
    ...
    >
      ...
      {/* Content Container */}
      <div className={cn("relative z-10 p-6 flex flex-col h-full", span && !isLocked ? "justify-end" : "justify-between")}>
        ...
        <div className="flex justify-between items-start mb-4">
          {!span && (
            <div className={cn(
                ...
                isLocked ? "bg-stone-200 text-stone-400" : "bg-stone-100 text-stone-700 group-hover:bg-boun-gold group-hover:text-white"
            )}>
                {icon}
            </div>
          )}
        ...
        {/* Text Content */}
        <div className={cn(span && !isLocked ? "text-white" : "text-stone-900")}>
          ...
          {description && (
            <p className={cn(
                ...
                span && !isLocked ? "text-stone-300" : "text-stone-500"
            )}>
              {description}
            </p>
          )}
          
          {/* CTA Button (Only active) */}
          {!isLocked && (
            <div className={cn(
